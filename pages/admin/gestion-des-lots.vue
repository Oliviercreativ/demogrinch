      ...
      <div class="bg-blue-50 p-4 rounded-lg">
        ...
        <div class="mt-4 flex justify-center">
          <button
            ...
            class="bg-blue-500 text-white font-bold py-2 px-6 rounded focus:outline-none focus:shadow-outline disabled:opacity-50"
          >
...
function ajouterLot() {
  if (nouveauLot.value.nom && nouveauLot.value.quantite > 0) {
    ...
    localStorage.setItem('jeuconcours_lots', JSON.stringify(lots.value))
...
function reinitialiserJeu() {
  if (confirm('Êtes-vous sûr de vouloir réinitialiser le jeu ? Cela effacera toutes les participations et les gagnants.')) {
    localStorage.removeItem('jeuconcours_gagnants')
    localStorage.removeItem('jeuconcours_participation')
    localStorage.removeItem('jeuconcours_participations_utilisateur')
...
function sauvegarderMaxParticipations() {
  localStorage.setItem('jeuconcours_max_participations', maxParticipations.value.toString())
}
</script>
